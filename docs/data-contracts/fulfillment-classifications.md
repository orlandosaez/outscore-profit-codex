...
The SLA detection branch reads `profit_sla_breached_candidates` (migration 030, **rewritten in migration 034b — V0.7.B.4**). The V0.7.B.4 rewrite changes the SOURCE: instead of filtering `profit_sla_service_items` (which only tracks services with revenue events), the candidate view now sources from `profit_anchor_services_attributed`, which covers EVERY active Anchor service. SLA state (`breached` / `at_risk` / `on_track` / `not_applicable`) is computed inline (CASE expression mirroring `profit_sla_service_items`'s state machine, minus workflow_status integration which remains deferred to V0.7.D). The view continues to exclude rows where `default_sla_day IS NULL`, `target_sla_day IS NULL`, or `target_date IS NULL`. The `sla_task_complete` clearance fires when a matching FC task has `is_completed = true OR completed_at IS NOT NULL`; the `sla_project_archived` clearance fires when a matching FC project has `is_closed = true OR closed_at IS NOT NULL`. Both clearance branches use no-op resolution because the existing taxonomy has no clean "work-complete-outside-billing" successor; `SETTLED_VIA_QUICKBOOKS_PAYMENT` is QBO-cash-specific and is not appropriate here.

V0.7.B.4 also applies Orlando's labeled-service attribution rule via `profit_anchor_services_attributed` (migration 034a): when an Anchor service name carries a parsed label (e.g., `"1065 Essential - NDH Holdings LLC"`), the SLA classification is attributed to the LABELED FC client (if the 4-strategy fuzzy resolver maps it) rather than the agreement holder. Unresolved labels stay attributed to the agreement holder with `label_unresolved = TRUE` for operator triage. The V0.7.B.3 content-specific `1040-on-business` regex (migration 031) is REMOVED by this rewrite; the data-driven labeled-service rule supersedes it. See [Weekly Review Data Contract: Attribution Layer](weekly-review.md#attribution-layer-v07b4) for the full attribution semantics.
