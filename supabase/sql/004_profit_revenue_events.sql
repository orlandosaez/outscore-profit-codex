create table if not exists profit_revenue_events (
  revenue_event_key text primary key,
  anchor_line_item_id text,
  ...
  candidate_period_month date,
  recognition_date date,
  recognition_period_month date,
  trigger_source text,
...
create index if not exists idx_profit_revenue_events_period_relationship_macro
  on profit_revenue_events (recognition_period_month, anchor_relationship_id, macro_service_type);

create index if not exists idx_profit_revenue_events_candidate_period
  on profit_revenue_events (candidate_period_month);

create index if not exists idx_profit_revenue_events_status
  on profit_revenue_events (recognition_status);

create index if not exists idx_profit_revenue_events_invoice
  on profit_revenue_events (anchor_invoice_id);

create or replace view profit_client_service_monthly_gp_recognition_basis as
with revenue as (
  select
    event.recognition_period_month as period_month,
    event.anchor_relationship_id,
    event.macro_service_type,
    sum(event.recognized_amount)::numeric as recognized_revenue_amount,
    count(*)::integer as revenue_event_count
  from profit_revenue_events event
  where event.recognition_period_month is not null
  group by 1, 2, 3
),
labor as (
  select
...
combined as (
  select
    ...
    coalesce(revenue.recognized_revenue_amount, 0)::numeric as recognized_revenue_amount,
    coalesce(revenue.revenue_event_count, 0)::integer as revenue_event_count,
...
select
  ...
  combined.recognized_revenue_amount,
  combined.matched_labor_cost,
  (combined.recognized_revenue_amount - combined.matched_labor_cost)::numeric as gp_amount,
  ((combined.recognized_revenue_amount - combined.matched_labor_cost) / nullif(combined.recognized_revenue_amount, 0))::numeric as gp_pct,
  combined.matched_hours,
  combined.matched_time_entry_count,
  combined.revenue_event_count,
  'recognition_basis'::text as basis
from combined
...
create or replace view profit_company_monthly_gp_recognition_basis as
with revenue as (
  select
    event.recognition_period_month as period_month,
    sum(event.recognized_amount)::numeric as recognized_revenue_amount
...
select
  ...
  (recognized_revenue_amount - contractor_labor_cost)::numeric as gp_amount,
  ((recognized_revenue_amount - contractor_labor_cost) / nullif(recognized_revenue_amount, 0))::numeric as gp_pct,
  total_hours,
  admin_hours,
...
create or replace view profit_revenue_event_status_summary as
select
  coalesce(candidate_period_month, recognition_period_month) as period_month,
  macro_service_type,
  recognition_status,
  recognition_rule,
  count(*)::integer as event_count,
  sum(source_amount)::numeric as source_amount,
  sum(recognized_amount)::numeric as recognized_amount
