-- Migration 060: Lower L.2 client-match audit-only trigram band from 0.85 to 0.55.
--
-- Lesson from 2026-05-24 walkthrough: FC "B&B Technology Inc.
-- (SimpleSwitch.io)" / "B&B Technologies, Inc." and Anchor "B&B Technology
-- Solutions" normalized close enough for a human but only scored about 0.59
-- trigram similarity. The old audit-only band (0.85 <= trigram < 0.92)
-- silently missed this class of likely client-match gap.
--
-- One predicate threshold change only: L.2 audit-only candidates now surface
-- when 0.55 <= trigram < 0.92. No auto_fuzzy threshold or row-shape
-- changes. Carries forward every prior category (A-K, pipeline_cron_stale,
-- L1/L2/L3, M, N).
...
comment on view profit_data_quality_alerts is
  'V0.7.N (060): 15 alert categories. Lowers L.2 client-match audit-only trigram band to 0.55 <= similarity < 0.92 so B&B-class mismatches surface for operator review; auto_fuzzy remains >= 0.92 candidate-only. Carries forward all prior categories. See migration headers 053 (L1/L2/L3 client matching), 054 (E frequency-aware + M held-invoice), 056 (F operator naming convention), 057 (N engagement aged), and 050 (pipeline_cron_stale).';
...
begin
  if to_regclass('public.profit_data_quality_alerts') is null then
    raise exception '060 verify FAIL: profit_data_quality_alerts missing';
  end if;

  raise notice '060 verify: schema OK';
end $$;
