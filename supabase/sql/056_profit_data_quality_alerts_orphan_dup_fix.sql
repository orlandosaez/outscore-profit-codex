-- Migration 056: Fix category F (orphan_attribution_duplicate) false positive
-- on operator's parent + subsidiary naming convention.
--
-- Discovered 2026-05-24: DVH Investing LLC agreement legitimately has both
-- "1065 Essential" (unlabeled, applies to DVH itself) AND "1065 Essential -
-- NDH Holdings LLC" (labeled, applies to subsidiary NDH). Operator's
-- convention across many agreements: unlabeled service = applies to the
-- business named in the agreement; labeled service = applies to a different
-- entity. These are two DIFFERENT filings, NOT duplicates.
--
-- Pre-056 category F joined explicitly on "one labeled + one unlabeled with
-- same canonical service" — exactly the parent+subsidiary pattern. False
-- positive by construction.
--
-- Post-056: only fire when 2+ rows share the SAME (relationship_id,
-- canonical_service, label) tuple. NULL label matches NULL label (real
-- duplicate of the primary-entity service). Different labels — including
-- one NULL + one non-NULL — are treated as distinct entities and do NOT
-- trigger.
--
-- Carries forward every 054 category (A-K, pipeline_cron_stale, L1/L2/L3, M)
-- in a single CREATE OR REPLACE VIEW per the 053 hotfix lesson.
...
comment on view profit_data_quality_alerts is
  'V0.7.K + 056 hotfix: 14 alert categories. Category E subscription_billing_gap uses nested service.billing.trigger/occurrence/status with monthly/quarterly/safety-net thresholds 45/100/380 days. Category M held_invoice_unpaid surfaces unpaid active-agreement Anchor invoices older than 30 days. Category F orphan_attribution_duplicate (056 hotfix) honors operator parent/subsidiary naming convention: only fires when 2+ services share SAME (canonical_service, label) tuple — NULL=NULL counts as duplicate-of-primary, NULL+label or label+label are distinct entities and do NOT fire. Carries forward 053 categories A-K, pipeline_cron_stale, and L client_match_suspected_dup_or_gap.';
...
begin
  if to_regclass('public.profit_data_quality_alerts') is null then
    raise exception '056 verify FAIL: profit_data_quality_alerts missing';
  end if;

  raise notice '056 verify: schema OK';
end $$;
