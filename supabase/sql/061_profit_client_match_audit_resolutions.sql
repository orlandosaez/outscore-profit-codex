-- Migration 061: Audit-resolution suppression for L.2 client-match alerts.
--
-- After V0.7.N (060) lowered the L.2 audit-only band to 0.55, 4 borderline
-- pairs surfaced (JBLD↔LJD Performance Transmissions, Samdee RE↔Samdee
-- Enterprises, Maria Schaffner). Orlando triaged each and confirmed:
--   - JBLD ↔ LJD: distinct legal entities with similar names (don't merge)
--   - Samdee: related but distinct entities (don't merge)
--   - Maria Schaffner: same person (operator merged in FC; self-clears via W17)
--
-- For the "don't merge but stop alerting on this pair" cases, this migration
-- introduces profit_client_match_audit_resolutions — operator-managed table
-- recording per-(fc_client_id, anchor_relationship_id) pair acknowledgments.
-- L.2 predicate excludes any pair with a matching resolution row.
--
-- This is the third operator-managed escape hatch in the matching layer:
--   1. profit_client_aliases — "this name maps to this FC client" (V0.7.J)
--   2. manual_override match_method — promoted candidate (pre-V0.7.J)
--   3. profit_client_match_audit_resolutions — "ack, distinct, stop alerting" (this migration)
--
-- Carries forward every prior category (A-K, pipeline_cron_stale, L1/L2/L3,
-- M, N) per the 053 hotfix lesson.

create table if not exists profit_client_match_audit_resolutions (
  fc_client_id bigint not null references profit_fc_clients(fc_client_id),
  anchor_relationship_id text not null,
  resolution text not null check (
    resolution in (
      'separate_entities',       -- similar names, but legally distinct entities
      'related_but_distinct',    -- parent/subsidiary or affiliated, but billed separately
      'duplicate_acknowledged'   -- known duplicate kept intentionally for ops reasons
    )
  ),
  note text,
  resolved_at timestamptz not null default now(),
  resolved_by text not null default 'orlando',
  primary key (fc_client_id, anchor_relationship_id)
);

comment on table profit_client_match_audit_resolutions is
  'V0.7.O (061) operator acknowledgments suppressing L.2 audit alerts for pairs the operator has reviewed and confirmed should NOT be merged. Three reasons supported: separate_entities (similar names, distinct legal entities), related_but_distinct (parent/sub/affiliated, billed separately), duplicate_acknowledged (kept duplicate intentionally). Pair = (fc_client_id, anchor_relationship_id); primary key prevents duplicate resolutions.';
...
select distinct 'label_unresolved_with_sibling_candidate', 'medium', 'service_line',
  ...
  'Unresolved label "' || asa.label || '" on service "' || asa.canonical_service_name
    ...
    || '" in same group has token overlap but no strategy auto-resolves. '
    || 'To clear: rename the Anchor label to literally "' || regexp_replace(sibling.name, '\s*\([^)]*\)\s*$', '')
    || '" (matches the FC display name; strategy 1 exact normalized then resolves). '
    || 'Alternatively, rename the FC client to match the current Anchor label.',
...
where fc.is_archived = false
  ...
  -- V0.7.O (061): suppress pairs the operator has reviewed + acknowledged
  -- as legitimately distinct (separate_entities / related_but_distinct /
  -- duplicate_acknowledged). Resolution rows live in
  -- profit_client_match_audit_resolutions.
  and not exists (
    select 1
    from profit_client_match_audit_resolutions r
    where r.fc_client_id = fc.fc_client_id
      and r.anchor_relationship_id = ag.anchor_relationship_id
  )
...
comment on view profit_data_quality_alerts is
  'V0.7.O (061): 15 alert categories. Adds L.2 suppression via profit_client_match_audit_resolutions — operator records pairs reviewed and confirmed as legitimately distinct (separate_entities / related_but_distinct / duplicate_acknowledged) and L.2 stops alerting on those pairs. Previous V0.7.N (060) lowered L.2 band to 0.55 <= sim < 0.92; auto_fuzzy remains >= 0.92 candidate-only. Carries forward all prior categories. See migration headers 053 (L1/L2/L3), 054 (E + M), 056 (F), 057 (N), 050 (pipeline_cron_stale).';
...
begin
  if to_regclass('public.profit_data_quality_alerts') is null then
    raise exception '061 verify FAIL: profit_data_quality_alerts missing';
  end if;

  if not exists (
    select 1 from pg_tables where tablename = 'profit_client_match_audit_resolutions'
  ) then
    raise exception '061 verify FAIL: profit_client_match_audit_resolutions table missing';
  end if;

  raise notice '061 verify: schema OK';
end $$;
