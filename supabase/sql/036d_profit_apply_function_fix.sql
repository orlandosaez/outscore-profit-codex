-- Migration 036d: V0.7.D-2 apply function fix — corrects profit_anchor_agreements join
--
-- Codex's 036b introduced a manual_invoice_no_active_manual_trigger_services_signals
-- CTE that joined `profit_anchor_agreements active_agreement` directly on
-- `active_agreement.fc_client_id` — but profit_anchor_agreements has no
-- fc_client_id column. The mapping from FC client → Anchor agreement lives
-- in profit_fc_client_anchor_matches.
--
-- The error doesn't surface at CREATE FUNCTION time (the function body
-- parses fine); it fires only when profit_apply_classification_transitions
-- is invoked. Smoke gate's BEGIN/ROLLBACK only validates DDL, not runtime.
--
-- This migration re-CREATE OR REPLACEs the function with the corrected
-- predicate. All other CTEs preserved verbatim from 036b.
--
-- Fix:
--   - from profit_anchor_agreements active_agreement
--     where active_agreement.fc_client_id = current_classifications.fc_client_id
--   + from profit_fc_client_anchor_matches match_a
--     join profit_anchor_agreements active_agreement
--       on active_agreement.anchor_relationship_id = match_a.anchor_relationship_id
--     where match_a.fc_client_id = current_classifications.fc_client_id
--
-- No other change. Depends on: 036b (function body source).
-- Predeploy_smoke.sh gate must pass; runtime validation via
-- `select * from profit_apply_classification_transitions(now(), true)` after apply.
...
begin
  ...
  for transition_record in
    ...
    manual_invoice_no_active_manual_trigger_services_signals as (
      select
        current_classifications.classification_id,
        current_classifications.fc_client_id,
        current_classifications.fc_client_name,
        ...
        current_classifications.anchor_client_business_name,
        jsonb_build_object(
          'fc_client_id', current_classifications.fc_client_id,
          'anchor_relationship_id', current_classifications.anchor_relationship_id,
          'resolution', 'no-op resolution',
          'reason', 'no active agreement carrying a manual-trigger service'
        ) as evidence_summary,
        p_run_at as last_signal_at,
        current_classifications.source_audit_file,
        current_classifications.source_audit_row_hash,
      ...
      join profit_classification_transition_rules rule
        on rule.from_verdict_code = current_classifications.from_verdict_code
       and rule.signal_name = 'manual_invoice_no_active_manual_trigger_services'
       and rule.enabled = true
      where current_classifications.from_verdict_code = 'MANUAL_INVOICE_PENDING'
        and current_classifications.fc_client_id is not null
        and not exists (
          select 1
          from profit_fc_client_anchor_matches match_a
          join profit_anchor_agreements active_agreement
            on active_agreement.anchor_relationship_id = match_a.anchor_relationship_id
          where match_a.fc_client_id = current_classifications.fc_client_id
            and active_agreement.display_status = 'active'
            and active_agreement.raw->'profitSyncServiceSummary' @> '[{"trigger":"manual"}]'::jsonb
        )
    ),
    ...
    ranked_signals as (
      ...
      select *
      from manual_invoice_no_active_manual_trigger_services_signals
