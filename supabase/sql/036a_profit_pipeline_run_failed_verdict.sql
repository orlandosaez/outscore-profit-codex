-- Migration 036a: PIPELINE_RUN_FAILED verdict
-- V0.7.D-2 Task 6. Adds system-health queue verdict, candidate view,
-- and transition-function branches.
...
insert into profit_classification_verdicts (
  ...
  ('PIPELINE_RUN_FAILED', 'Pipeline run failed', 'pending', 'show', false, true, 'Pipeline run finished failed, or partial with failed steps, and no later successful run has cleared it.')
...
insert into profit_classification_transition_rules (
  ...
  ('PIPELINE_RUN_FAILED', 'pipeline_run_failed_detected', 'PIPELINE_RUN_FAILED', false, true, 'Self-detect rule used by profit_apply_classification_transitions to insert one active classification per unresolved failed or partial pipeline_run_id.'),
  ('PIPELINE_RUN_FAILED', 'pipeline_run_succeeded_after_failure', 'PIPELINE_RUN_FAILED', false, true, 'No-op resolution marker. Existing taxonomy has no healthy-system successor verdict, so the apply function supersedes the active row with superseded_by_classification_id = null when a later profit_pipeline_runs row finishes status=success.')
...
with latest_success as (
  select max(success.finished_at) as finished_at
  from profit_pipeline_runs success
...
failed_steps as (
  select
    ...
    string_agg(step.step_name, ', ' order by step.step_order, step.step_name) filter (where step.status = 'failed') as failed_step_names
  from profit_pipeline_run_steps step
  group by step.pipeline_run_id
)
select
  ...
  '/admin/audit/pipeline-runs/' || run.pipeline_run_id::text as action_url,
  run.pipeline_run_id,
  run.status,
  run.started_at,
  run.finished_at,
...
left join active_pipeline_classification
  on active_pipeline_classification.pipeline_run_id = run.pipeline_run_id::text
left join failed_steps
  on failed_steps.pipeline_run_id = run.pipeline_run_id
where (
    run.status = 'failed'
    or (
      run.status = 'partial'
      and (run.summary->>'total_steps_failed')::integer > 0
    )
  )
  and (
    latest_success.finished_at is null
    or run.started_at > latest_success.finished_at
  );

comment on view profit_pipeline_run_failed_candidates is
  'V0.7.D-2 system-health candidate view. Surfaces one row per failed pipeline_run_id, or partial with summary.total_steps_failed > 0, when no later successful run has cleared it. No time window.';
...
begin
  ...
  for transition_record in
    ...
    current_classifications as (
      select
        ...
        case
          ...
          when classification.source_audit_file = 'system:pipeline_run_failed' then split_part(classification.source_audit_row_hash, ':', 2)
          else null::text
        end as service_name
      from profit_classifications classification
      left join profit_fc_clients client
    ...
    pipeline_run_failed_detected_signals as (
      select
        null::bigint as classification_id,
        null::bigint as fc_client_id,
        ...
        null::bigint as group_id,
        null::text as from_verdict_code,
        'pipeline_run_failed_detected'::text as signal_name,
        'PIPELINE_RUN_FAILED'::text as to_verdict_code,
        null::text as anchor_relationship_id,
        candidate.client_name as anchor_client_business_name,
        jsonb_build_object(
          ...
          'failed_step_names', candidate.failed_step_names
        ) as evidence_summary,
        coalesce(candidate.finished_at, candidate.started_at, p_run_at) as last_signal_at,
        'system:pipeline_run_failed'::text as source_audit_file,
        'pipeline_run_failed:' || candidate.pipeline_run_id::text as source_audit_row_hash,
        null::numeric as estimated_annual_revenue,
        candidate.pipeline_run_id::text as service_name,
        8 as signal_priority
      from profit_pipeline_run_failed_candidates candidate
      where candidate.classification_id is null
    ),
    pipeline_run_succeeded_after_failure_signals as (
      select
        current_classifications.classification_id,
        current_classifications.fc_client_id,
        current_classifications.fc_client_name,
        ...
        jsonb_build_object(
          ...
          'failed_status', failed_run.status,
          'failed_finished_at', failed_run.finished_at,
          'success_pipeline_run_id', later_success.pipeline_run_id,
          'success_finished_at', later_success.finished_at,
          'resolution', 'no-op resolution'
        ) as evidence_summary,
        later_success.finished_at as last_signal_at,
        current_classifications.source_audit_file,
        current_classifications.source_audit_row_hash,
        current_classifications.estimated_annual_revenue,
        current_classifications.service_name,
        17 as signal_priority
      from current_classifications
      join profit_classification_transition_rules rule
        on rule.from_verdict_code = current_classifications.from_verdict_code
       and rule.signal_name = 'pipeline_run_succeeded_after_failure'
       and rule.enabled = true
      join profit_pipeline_runs failed_run
        on failed_run.pipeline_run_id = current_classifications.service_name::uuid
      join profit_pipeline_runs later_success
        on later_success.status = 'success'
       and later_success.finished_at > failed_run.finished_at
      where current_classifications.from_verdict_code = 'PIPELINE_RUN_FAILED'
        and current_classifications.service_name is not null
    ),
    ...
    ranked_signals as (
      ...
      select *
      from pipeline_run_failed_detected_signals
      ...
      select *
      from pipeline_run_succeeded_after_failure_signals
    ...
    select distinct on (
      coalesce(
        ...
        case
          ...
          when ranked_signals.from_verdict_code is null and ranked_signals.signal_name = 'pipeline_run_failed_detected'
          then 'detect:pipeline_run_failed:' || ranked_signals.service_name
