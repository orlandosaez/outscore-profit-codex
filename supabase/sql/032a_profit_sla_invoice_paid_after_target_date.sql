-- Migration 032a: Tighten sla_invoice_paid clearance to invoice issued at-or-after target
-- V0.7.B.3 audit fix continuation — 032 was too aggressive
--
-- Live verification of 032 showed 7 of 10 sampled cleared rows had paid
-- invoices issued BEFORE the SLA target date — invoices from prior billing
-- cycles. Lee's Ice paid April's Accounting Plus on April 28, but May's
-- target was 2026-05-10; the April invoice doesn't mean May's work is done.
-- Same pattern for Payroll Tax Compliance monthly cycles.
--
-- Fix: tighten the invoice-paid predicate (both in candidate view and apply
-- function clearance CTE) to require:
--   invoice.issue_date::date >= item.target_date
--
-- Tax services (1120, 1065, 1040): target_date = March 15 deadline, invoice
-- paid April/May AFTER target. Correctly cleared.
-- Monthly accounting / payroll: target_date = end of period, invoice
-- arrears-paid after period end. Correctly cleared.
-- Prior-period payments: invoice paid BEFORE current period's target.
-- Correctly NOT cleared.
--
-- Edge case acknowledged: prepaid-upfront annual services have invoice paid
-- BEFORE target. The work is done (paid upfront) but rule would not clear.
-- Rare in current data; address when first hit.
--
...
with source_items as (
  ...
  where item.sla_state in ('breached', 'at_risk')
    ...
    -- V0.7.B.3 audit gap closure (tightened in 032a): exclude items whose
    -- Anchor agreement has a paid invoice issued AT OR AFTER the SLA target
    -- date. 032's untightened rule cleared 7 of 10 sampled rows where the
    -- paid invoice was from a prior billing cycle (e.g., Lee's Ice paid
    -- April's Accounting Plus but May's target was 2026-05-10 — the April
    -- invoice doesn't mean May's work is done). The issue_date >= target_date
    -- predicate catches tax returns (paid weeks after deadline) and arrears-
    -- billed monthly services (paid after period end). Coarse per-agreement
    -- still — invoice line items not parsed; V0.7.D Anchor sync expansion.
    and not exists (
      select 1
      ...
      where invoice.anchor_relationship_id = item.anchor_relationship_id
        ...
        and invoice.issue_date::date >= item.target_date
    )
  order by
    item.anchor_relationship_id,
    item.service_name,
...
comment on view profit_sla_breached_candidates is
  'V0.7.B.3 audit fix (032a): excludes 1040* services on business-suffix clients (LLC/Inc/Corp/LLP/PA, word-boundary anchored). Excludes items where the same Anchor agreement has a paid invoice issued AT OR AFTER the SLA target_date (tightened in 032a after 032 over-cleared 7 of 10 sampled rows by matching prior-billing-cycle invoices). V0.7.B.1 T6a still active: excludes items where matching FC task is complete or matching FC project is closed. source_audit_row_hash key sla_breached:<fc_client_id>:<service_name>. action_url falls back FC task -> FC project -> Anchor. waiting_on_client + staff fallback + 1120 entity_type + invoice line items all deferred to V0.7.D.';
  ...
  for transition_record in
    ...
    sla_invoice_paid_signals as (
      ...
      join profit_classification_transition_rules rule
       ...
       and rule.enabled = true
      -- V0.7.B.3 audit fix (032a): join target_date from profit_sla_service_items
      -- via the classification's (fc_client_id, service_name) so the invoice
      -- predicate can require issue_date >= target_date. Without this lateral,
      -- the clearance branch can't know the SLA target date for the
      -- classification's specific service.
      left join lateral (
        select item.target_date
        from profit_sla_service_items item
        where item.fc_client_id = current_classifications.fc_client_id
          and item.service_name = current_classifications.service_name
        order by item.target_date asc nulls last
      ...
      join profit_anchor_invoices invoice
       ...
       and service_item.target_date is not null
       and invoice.issue_date::date >= service_item.target_date
      where current_classifications.from_verdict_code = 'SLA_BREACHED'
        and current_classifications.service_name is not null
      group by
        current_classifications.classification_id,
