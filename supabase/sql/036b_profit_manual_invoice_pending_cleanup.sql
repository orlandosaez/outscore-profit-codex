-- Migration 036b: stale MANUAL_INVOICE_PENDING cleanup
-- V0.7.D-2 Task 8. Adds a no-successor cleanup signal for active manual
-- invoice classifications whose FC client no longer has any active Anchor
-- agreement carrying a manual-trigger service.
...
insert into profit_classification_transition_rules (
  ...
  ('MANUAL_INVOICE_PENDING', 'manual_invoice_no_active_manual_trigger_services', 'MANUAL_INVOICE_PENDING', false, true, 'No-op resolution marker. Existing taxonomy has no stale-manual-invoice successor verdict, so the apply function supersedes the active row with superseded_by_classification_id = null when the FC client has no active agreement carrying a manual-trigger service summary.')
