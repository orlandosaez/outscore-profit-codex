-- Migration 035h: SLA candidates use FC custom_fields as primary staff source (V0.7.D-1.1)
--
-- As of Orlando's 2026-05-12 directive, FC custom fields (Tax Preparer,
-- Tax Reviewer, Book Primary, Book Reviewer) are the authoritative source
-- for staff assignments. This migration shifts profit_sla_breached_candidates
-- to read from profit_fc_client_staff_from_custom_fields (migration 035g)
-- BEFORE falling back to:
--   1. The XLSX-seeded profit_client_staff_assignments table (035e/035f) —
--      now transitional, queued for deprecation
--   2. The derived primary-preparer view (035a/035d) — fallback for clients
--      with no FC custom_field value AND no XLSX seed
--
-- Lookup priority:
--   1. FC custom_fields via 035g view (NEW PRIMARY)
--        - For tax SLAs (non-bookkeeping fc_tag): Tax Preparer
--        - For bookkeeping SLAs (fc_tag like 'S BOOK%'): Book Primary
--   2. XLSX-seeded assignments via 035e/035f (transitional)
--   3. Derived from FC tasks via 035a/035d (final fallback)
--
-- The 035g source distinguishes Tax vs Book based on the SLA's fc_tag
-- prefix, so a 1040 SLA reads Tax Preparer; an Accounting Plus SLA reads
-- Book Primary. This matches Orlando's 4-field model exactly.
--
-- staff_source values widen to:
--   'fc_custom_field' — preferred, FC is the source
--   'authoritative_assignment' — XLSX-seed (transitional)
--   'derived_primary_preparer' — FC task-assignee derivation (fallback)
--   'unassigned' — none of the above resolved
--
-- Column order preserved (26 columns, V0.7.B.4 contract + 035b + 035d + 035f).
...
comment on view profit_sla_breached_candidates is
  'V0.7.D-1.1 (035h): assigned_staff_name reads FC custom_fields (Tax Preparer / Book Primary based on fc_tag) as authoritative; falls back to XLSX-seeded authoritative_assignment (035e/035f, transitional cache, queued for deprecation), then derived FC-task primary-preparer (035a/035d). staff_source widens to include ''fc_custom_field''. Column order preserved (26 columns, V0.7.B.4 contract).';
