-- Migration 054: V0.7.K frequency-aware billing audit.
--
-- Rewrites category E (subscription_billing_gap) to use Anchor's nested
-- per-service billing taxonomy and adds category M (held_invoice_unpaid).
-- Carries forward every 053 category in a single CREATE OR REPLACE VIEW.
...
comment on view profit_data_quality_alerts is
  'V0.7.K: 14 alert categories. Category E subscription_billing_gap is frequency-aware using nested service.billing.trigger/service.billing.occurrence/service.status.type paths with monthly/quarterly/safety-net thresholds of 45/100/380 days. Category M held_invoice_unpaid surfaces unpaid active-agreement Anchor invoices older than 30 days. Carries forward 053 categories A-K, pipeline_cron_stale, and L client_match_suspected_dup_or_gap.';
...
begin
  if to_regclass('public.profit_data_quality_alerts') is null then
    raise exception '054 verify FAIL: profit_data_quality_alerts missing';
  end if;

  raise notice '054 verify: schema OK';
end $$;
