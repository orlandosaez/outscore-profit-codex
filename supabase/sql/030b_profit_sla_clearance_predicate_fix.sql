-- Migration 030b: SLA clearance ILIKE predicate fix
-- V0.7.B.1 Task 3 - Correctness + Trust Restoration
--
-- Fixes F1 from UX audit: Kodiak Enterprises LLC + Legacy Bids LLC stuck in
-- SLA_BREACHED despite completed FC tasks (2026-04-30).
--
-- Root cause: migration 030's sla_task_complete and sla_project_archived
-- clearance branches required the FULL service_name (e.g. '1120 Plus') to
-- appear in FC project titles (e.g. 'O 1120 Tax Return'). The strict ILIKE
-- never matched.
--
-- Fix: split_part(service_name, ' ', 1) extracts the form prefix ('1120',
-- '1065', 'Bookkeeping', etc.) which DOES appear in FC project titles.
--
-- This is the ONLY change vs migration 030's profit_apply_classification_transitions.
-- All signal names, priorities, dedup keys, and no-op resolution behavior
-- are preserved bit-for-bit.
--
-- Live verification gate: this migration must pass scripts/predeploy_smoke.sh
-- (psql --single-transaction BEGIN/ROLLBACK) BEFORE commit. The V0.7.B
-- project.project_title schema-mismatch bug taught us static SQL tests don't
-- exercise actual execution.
--
-- Depends on: 030 (function definition copied verbatim; only ILIKE
-- predicates differ).
