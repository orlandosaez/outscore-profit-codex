-- Migration 057: V0.7.L add category N (engagement_aged_without_fc_project).
--
-- Operator workflow rule (captured 2026-05-24, Orlando):
--   "Bill-at-delivery is standard for recurring engagements (tax returns)
--    with clients > 1 year. But every open Anchor agreement MUST have a
--    paired open FC project showing work in progress. Absence of that
--    pairing — engagement signed but no FC project ever opened — is the
--    red flag."
--
-- This audit pairs TWO independent system signals for high precision:
--   - Anchor signal: agreement has manual:yearly:upfront service(s),
--     zero invoices ever, signed > 120 days ago
--   - FC signal: no open project for the matched FC client
--
-- Either signal alone is noisy (zero invoices on a tax engagement is normal
-- mid-season; no FC project is normal if work hasn't started yet). The
-- pairing of both means: client engaged with us, agreement aged enough
-- that work should be underway, yet no work-in-progress exists in FC.
--
-- Calibration on prod (2026-05-24): 0 firings today. Bachert (77d),
-- Lee's Inc (26d), West Coast (102d) all have open FC projects → audit
-- correctly recognizes them as normal in-progress engagements.
--
-- Carries forward every 056 category (A-K, pipeline_cron_stale, L1/L2/L3,
-- M) in a single CREATE OR REPLACE VIEW per the 053 hotfix lesson.
...
begin
  if to_regclass('public.profit_data_quality_alerts') is null then
    raise exception '057 verify FAIL: profit_data_quality_alerts missing';
  end if;

  raise notice '057 verify: schema OK';
end $$;
