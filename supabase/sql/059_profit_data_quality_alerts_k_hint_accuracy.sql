-- Migration 059: Improve category K (label_unresolved_with_sibling_candidate)
-- description text to be actionable rather than misleading.
--
-- Lesson from 2026-05-24 walkthrough: K's old description said "sibling FC
-- client '<name>' in same group has token overlap. Likely target." That
-- text led Orlando to rename Anchor label "Steven Hadar" → "Hadar Steven"
-- thinking it would resolve. It didn't — the actual label-resolution layer
-- runs 4 strategies (exact normalized, prefix, last-token reorder, comma
-- + first-letter tolerance) and NONE accept "Hadar Steven" when the FC
-- client name is "Feig, Hadar Steven and Leora (1040)". Eventually Orlando
-- renamed to literally "Feig, Hadar Steven and Leora" which unlocked
-- strategy 1 (exact normalized match).
--
-- The hint text now spells out the actionable fix: rename the Anchor label
-- to literally match the FC client display name (drop any "(1040)" or
-- similar parenthetical suffix), which is the most reliable form for
-- strategy 1 to resolve. Sibling FC client suggestion is preserved but
-- the operator instruction is concrete rather than implied.
--
-- One description text change. No predicate or row-shape changes. Carries
-- forward every prior category (A-K, pipeline_cron_stale, L1/L2/L3, M, N).
...
begin
  if to_regclass('public.profit_data_quality_alerts') is null then
    raise exception '059 verify FAIL: profit_data_quality_alerts missing';
  end if;

  raise notice '059 verify: schema OK';
end $$;
